...
class AssembleLivePhotoViewController: BaseLivePhotoViewController{
    ...
    // MARK: -  Viet
    private lazy var asssembleVietButton: UIButton = {
        let button = UIButton()
        button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle("Assemble Image", for: .normal)
        button.setTitleColor(.red, for: .normal)
        button.addTarget(self, action: #selector(handleShowAddButtonTapped), for: .touchUpInside)
        return button
    }()
    
    @objc func handleShowAddButtonTapped() {
        ...
        assemble(photo: photoURL!, video: sourceVideoPath)
    }
    
    func assemble(photo: URL, video: URL) {
        progressView.progress = 0
        Task {
            let livePhoto = try await LivePhotoViet.sharedInstance.assemble(photoURL:photo, videoURL:video) { [weak self] process in
                guard let self else { return }
                self.progressView.progress = Float(process)
            }
            Task { @MainActor in
...
//                    })
                ...
                LivePhoto.generate(from: livePhoto.0, videoURL: livePhoto.1, progress: { (percent) in
                    ...
                    print("DEBUG: \(livePhoto) and \(resources)")
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(asssembleVietButton)
        
        NSLayoutConstraint.activate([
            asssembleVietButton.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            asssembleVietButton.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -130),
        ])
    }
